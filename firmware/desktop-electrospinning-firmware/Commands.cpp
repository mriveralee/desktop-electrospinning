...
bool PREVIOUSLY_HV_ENABLED = true;
void Commands::checkForPeriodicalActions(bool allowNewMoves) {
    ...
    Extruder::manageTemperatures();
      ...
      if (Extruder::current->id == ES_ESPLA_EXTRUDER_ID) {
        // We subtract the offset here because the offset in the POSITIVE dir is negative
        float realZWithOffset = Printer::realZPosition() + 1 * Printer::offsetZ;
        bool isInHVBounds = ESExtruderAddons::isPositionInHVBounds(
            Printer::realXPosition(),
            Printer::realYPosition(), 
            realZWithOffset);
        if (!Printer::isHoming() && isInHVBounds) {
          ESExtruderAddons::enableHighVoltage(!PREVIOUSLY_HV_ENABLED);
          PREVIOUSLY_HV_ENABLED = true;
        } else if (!PrintLine::hasLines()) {
          ESExtruderAddons::disableHighVoltage();
        }
      } else {
            ESExtruderAddons::disableHighVoltage();
            PREVIOUSLY_HV_ENABLED = false;
      }
    #endif
    if(--counter250ms == 0) {
        if(manageMonitor)
