        ...
        if(decoupleTestRequired && act->isDecoupleFullOrHold() && Printer::isPowerOn()) // Only test when powered
        {
            ...
            else     // Phase 2: Holding temperature inside a target corridor
            {
                if(fabs(act->currentTemperatureC - act->targetTemperatureC) > DECOUPLING_TEST_MAX_HOLD_VARIANCE)   // failed test
                {
                    ...
                    act->lastDecoupleTest = time - act->decoupleTestPeriod + TEMP_DECOUPLING_TEST_INTERVAL; // once running test every second
                }
            }
        }
...
void Extruder::selectExtruderById(uint8_t extruderId) {
    ...
    // ESP_OFFSET Computation
    #if SUPPORT_ELECTROSPINNING 
      if (extruderId == ES_ESPLA_EXTRUDER_ID) {
        float currentZPosInMM = Printer::realZPosition();
        float zOffsetSteps = ESExtruderAddons::getESExtruderOffsetZInSteps(currentZPosInMM, Printer::isHomed());
        extruder[ES_ESPLA_EXTRUDER_ID].zOffset = zOffsetSteps;
      } 
    #endif 
    // End ESP_Offset
    ...
    if(extruderId >= NUM_EXTRUDER) {
	    ...
	    Com::printFLN(PSTR("SelectExtruder:"), static_cast<int>(extruderId));
    #endif
    if(Printer::isHomed() && extruder[extruderId].zOffset < Extruder::current->zOffset) { 
      // prevent extruder from hitting bed
    ...
    Extruder::current->tempControl.updateTempControlVars();
       ...
       Printer::offsetX = 0;
        ...
        DEBUG_ES_PRINT(Com::printF(PSTR("Printer X_Offset = "), Printer::offsetX));
        DEBUG_ES_PRINT(Com::println());

    #endif
    // This is where offsets get applied for the current extruder
    // Note that the units of the extruder offsets are in STEPS
    // Since invAxisStepsPerMM has units MM/STEPS ---> STEPS * MM/STEPS = MM
...
{
#if NUM_EXTRUDER > 0
#if MIXING_EXTRUDER || (SHARED_EXTRUDER_HEATER && SHARED_EXTRUDER_HEATER != ES_SHARED_HEATER_E1_AND_E2)
    extr = 0; // map any virtual extruder number to 0
#elif (SHARED_EXTRUDER_HEATER == ES_SHARED_HEATER_E1_AND_E2)
  if (extr == ES_PLA_EXTRUDER_ID || extr == ES_ESPLA_EXTRUDER_ID) {
    extr = ES_PLA_EXTRUDER_ID;
  }
#endif // MIXING_EXTRUDER
