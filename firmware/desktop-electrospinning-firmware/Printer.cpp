...
uint8_t Printer::setDestinationStepsFromGCode(GCode *com)
{
    ...
    {
        if(com->hasX()) {
          lastCmdPos[X_AXIS] = currentPosition[X_AXIS] = convertToMM(com->X) - coordinateOffset[X_AXIS];
        }
        if(com->hasY()) {
          lastCmdPos[Y_AXIS] = currentPosition[Y_AXIS] = convertToMM(com->Y) - coordinateOffset[Y_AXIS];
        }
        if(com->hasZ()) {
...
//#endif
    ...
    UI_CLEAR_STATUS
    Commands::printCurrentPosition(PSTR("homeAxis "));
  if(zaxis && Extruder::current->id == ES_ESPLA_EXTRUDER_ID) {
    Extruder::selectExtruderById(ES_PLA_EXTRUDER_ID);
    Extruder::selectExtruderById(ES_ESPLA_EXTRUDER_ID);
    DEBUG_ES_PRINT(Com::printF(PSTR("Printer Z_POS = "), Printer::realZPosition()));
    DEBUG_ES_PRINT(Com::printF(PSTR("Printer zOffsetSteps = "), ESExtruderAddons::getESExtruderOffsetZInSteps( Printer::realZPosition(), Printer::isHomed())));
  }
}
